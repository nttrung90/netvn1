...
export default async function EditPostPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  if (!post) notFound();
  return <>
    {saved && <p role="status" className="mb-4 rounded-xl border border-[#a6f4c5] bg-[#ecfdf3] px-4 py-3 text-sm text-[#027a48]">Đã lưu thay đổi.</p>}
    <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#d0d5dd] pb-6">
      <div>
        <Link href="/admin/posts" className="inline-flex items-center gap-1.5 text-sm font-bold text-[#475467] transition hover:text-[#4062ff]"><ArrowLeft size={16}/>Tất cả bài viết</Link>
        <p className="mt-5 text-xs font-bold uppercase tracking-[.17em] text-[#4062ff]">Biên tập</p>
      ...
      <DeleteButton action={deletePost.bind(null, post.id)} label="Xóa bài viết" className="rounded-xl border border-[#fecdca] px-4 py-3 text-sm hover:bg-[#fef3f2]" />
    </div>
